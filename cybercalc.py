from Tkinter import *
root = Tk()
root.wm_title("CyberCalc")
root.resizable(0,0)
f1=Frame(root)
f1.pack(fill=BOTH,expand=1)
f2=Frame(root,background="#FFFFFF")
f2.pack(fill=BOTH,expand=1)
f3=Frame(root,background="#434343")
f3.pack(fill=BOTH,expand=1)
expr_inp = Entry(f2,highlightthickness=0)
expr_inp.grid(row=1,columnspan=6,padx=(10,10),pady=(5,5),sticky=E+W)
b_7 = Button(f3, text="7",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_7.grid(row=1, column=1,columnspan=1,padx=(10,5),pady=(10,5),sticky=E+W)
b_8 = Button(f3, text="8",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_8.grid(row=1, column=2,columnspan=1,padx=(5,5),pady=(10,5),sticky=E+W)
b_9 = Button(f3, text="9",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_9.grid(row=1, column=3,columnspan=1,padx=(5,5),pady=(10,5),sticky=E+W)
b_div = Button(f3, text="/",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_div.grid(row=1, column=4,columnspan=1,padx=(5,5),pady=(10,5),sticky=E+W)
b_revert = Button(f3, text="AC",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_revert.grid(row=1, column=5,columnspan=1,padx=(5,5),pady=(10,5),sticky=E+W)
b_back = Button(f3, text="C",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_back.grid(row=1, column=6,columnspan=1,padx=(5,10),pady=(10,5),sticky=E+W)

b_4 = Button(f3, text="4",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_4.grid(row=2, column=1,columnspan=1,padx=(10,5),pady=(5,5),sticky=E+W)
b_5 = Button(f3, text="5",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_5.grid(row=2, column=2,columnspan=1,padx=(5,5),pady=(5,5),sticky=E+W)
b_6 = Button(f3, text="6",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_6.grid(row=2, column=3,columnspan=1,padx=(5,5),pady=(5,5),sticky=E+W)
b_mul = Button(f3, text="*",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_mul.grid(row=2, column=4,columnspan=1,padx=(5,5),pady=(5,5),sticky=E+W)
b_oc = Button(f3, text="(",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_oc.grid(row=2, column=5,columnspan=1,padx=(5,5),pady=(5,5),sticky=E+W)
b_cb = Button(f3, text=")",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_cb.grid(row=2, column=6,columnspan=1,padx=(5,10),pady=(5,5),sticky=E+W)

b_1 = Button(f3, text="1",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_1.grid(row=3, column=1,columnspan=1,padx=(10,5),pady=(5,5),sticky=E+W)
b_2 = Button(f3, text="2",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_2.grid(row=3, column=2,columnspan=1,padx=(5,5),pady=(5,5),sticky=E+W)
b_3 = Button(f3, text="3",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_3.grid(row=3, column=3,columnspan=1,padx=(5,5),pady=(5,5),sticky=E+W)
b_sub = Button(f3, text="-",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_sub.grid(row=3, column=4,columnspan=1,padx=(5,5),pady=(5,5),sticky=E+W)
b_sq = Button(f3, text="x^2",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_sq.grid(row=3, column=5,columnspan=1,padx=(5,5),pady=(5,5),sticky=E+W)
b_sqrt = Button(f3, text="SQRT",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_sqrt.grid(row=3, column=6,columnspan=1,padx=(5,10),pady=(5,5),sticky=E+W)

b_0 = Button(f3, text="0",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_0.grid(row=4, column=1,columnspan=1,padx=(10,5),pady=(5,10),sticky=E+W)
b_deci = Button(f3, text=".",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_deci.grid(row=4, column=2,columnspan=1,padx=(5,5),pady=(5,10),sticky=E+W)
b_per = Button(f3, text="%",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_per.grid(row=4, column=3,columnspan=1,padx=(5,5),pady=(5,10),sticky=E+W)
b_add = Button(f3, text="+",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica", 12))
b_add.grid(row=4, column=4,columnspan=1,padx=(5,5),pady=(5,10),sticky=E+W)
b_eq = Button(f3, text="=",bg="#434343",fg="#FFFFFF",highlightthickness=0,font=("Helvetica",12))
b_eq.grid(row=4, column=5,columnspan=2,padx=(5,10),pady=(5,10),sticky=E+W)

root.mainloop()

